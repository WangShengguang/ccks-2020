...
def train_qa():
    ...
    for question, sparql, answer in load_data(tqdm_prefix='test qa'):
        ...
        standard_subject_entities = entity_pattern.findall(sparql) + attr_pattern.findall(sparql)
        standard_answer_entities = entity_pattern.findall(answer) + attr_pattern.findall(answer)
        try:
        ...
        # ipdb.set_trace()
        data.append(
            [question, standard_subject_entities, list(candidate_entities), standard_answer_entities, result_entities])
    data_df = pd.DataFrame(data, columns=['question', 'standard_subject_entities', 'candidate_entities',
    ...
    with open(ResultSaver().submit_result_txt, 'w', encoding='utf-8') as f:
        for index, row in tqdm(data_df.iterrows(), total=data_df.shape[0], desc='qa answer'):
            ...
            try:
                ...
                result_entities = [ent if ent.startswith('<') else f'"{ent}"' for ent in result_entities]
...
def valid2submit():
    ...
    data_df = pd.read_csv(ResultSaver(find_exist_path=True).valid_result_csv[0])
    with open(ResultSaver().submit_result_txt, 'w', encoding='utf-8') as f:
...
def main():
    ...
    group.add_argument('--train_qa', action="store_true", help="训练集答案")
    group.add_argument('--valid_qa', action="store_true", help="验证数据（待提交数据）答案")
...
if __name__ == '__main__':
    """
    example:
        nohup  python qa.py --train_qa &>train_qa.out & 
        nohup  python qa.py --valid_qa &>valid_qa.out & 
