...
class Recognizer(object):
    ...
    def get_ner_words(self, text):
        ...
        for word, tag in zip(words, tags):
            if len(word) < 2 or word in self.entity_stop_words:
                continue
            if tag == 'MENTION' or word in self.memory.mention2entity:
                if word not in self.mention_stop_words:
                    mentions.append(word)
            elif tag == 'ENTITY' or f"<{word}>" in self.memory.entity2id:
    ...
    def get_entity_feature(self, q_text, ent_name):
        # 得到实体两跳内的所有关系
        one_hop_rel_names = self.graph_db.get_onehop_relations_by_entName(ent_name)
        two_hop_rel_names = self.graph_db.get_twohop_relations_by_entName(ent_name)
        rel_names = [rel for rels in two_hop_rel_names for rel in rels] + one_hop_rel_names
        # 计算问题和主语实体及其两跳内关系间的相似度
        # TODO relnames 有空格；注意过滤
        similar_feature = self.ent_rel_similar(q_text, ent_name, rel_names)
        # 实体的流行度特征
        popular_feature = self.graph_db.get_onehop_relCount_by_entName(ent_name)
        return similar_feature, popular_feature

    def get_candate_entities(self, q_text):
        """
        :param q_text:
